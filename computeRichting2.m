...
% matrix
% flag = 1 when the delivery man moves towards the target 
% flag = 2 when it encounters a turn
function [X_del, Y_del, flag] = computeRichting2(desDirMat, x, y, x_pr, y_pr, avDirMat, flag)

prodMat = desDirMat.*avDirMat
sum(prodMat(:))

% when only one desired direction is found
if sum(prodMat(:)) ==  1 & flag ~=1
    [r c] = ind2sub(size(prodMat), find(prodMat == 1));
    X_del = x+c-2;
    Y_del = y+r-2;
    
% when two possible directions are there
elseif (sum(prodMat(:)) == 2 & flag ~=1) | flag == 2
