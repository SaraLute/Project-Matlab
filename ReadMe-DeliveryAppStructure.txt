...
1. User making an order to a restaurant (Sara's scripts)
2. Selection of a delivery person based on the distance to the restaurant (Kaiyu's scripts)
3. Tracking the position of delivery person from origin to designated restaurant (Track Delivery.m + DeliveryManProgress.m)
4. Tracking the position of delivery person from restaurant to final address aka destination (Track Delivery.m + DeliveryManProgress.m)
5. Both the parts (3) and (4) report the time taken by the delivery person.

The hierarchy of the program is as follows:

-> main -  Parent program invokes the delivery app
	--> Function : setupBook.m : function that invokes the structure to save the data from the customer
	- Prompt user to make an account and place an order
	--> getOrder.m : funtion that ..... # Sara's edit
	--> findAccount.m
