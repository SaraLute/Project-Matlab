...
function [X_src Y_src] = TrackDelivery(X_dest, Y_dest, X_src, Y_src, idx, idy)

% distance based computation
    tmpVar_x_dist = abs(X_src-X_dest);
    tmpVar_y_dist = abs(Y_src-Y_dest);
    if (tmpVar_x_dist > tmpVar_y_dist | tmpVar_x_dist == tmpVar_y_dist) & idx ~=1
    ...
    elseif (tmpVar_y_dist > tmpVar_x_dist | tmpVar_x_dist == tmpVar_y_dist) & idy ~=1
